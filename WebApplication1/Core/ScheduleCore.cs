            ...
            try
            {
                ...
                if (ScheduleList.Count > 0)
                {
                    foreach (var recepient in ScheduleList)
                    {
                        ...
                        {
                            client.BaseAddress = new Uri(Constant.BaseApiURL);

                            var PostData = new FormUrlEncodedContent(new[]
                            {
                            new KeyValuePair<string, string>("dnis", ContactData),
                            new KeyValuePair<string, string>("message", Constant.TestMessage)
                        });

                            var result = await client.PostAsync(Constant.SendSMSApiURL, PostData);
                            ...
                            _iRepository.ExecuteQuery<Schedule>(modelObj, Constant.SpCreateUpdateSchedule).FirstOrDefault();
                        }
                    }
                }
            }
        ...
        public async Task<APIResponse> CheckSMSStatus(string messageId)
        {
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    var result = await client.GetStringAsync(Constant.CheckSMSApiURL + messageId);
